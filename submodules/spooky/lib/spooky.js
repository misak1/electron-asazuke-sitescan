...
var command = fs.realpathSync(__dirname + '/../bin/casperjs/casperjs' + (_platform == 'win32' ? '.exe' : ''));
...
var isJsonRpcRequest = function (s) {
    try {
        s = JSON.parse(s);
...
var isJsonRpcResponse = function (s) {
    try {
        s = JSON.parse(s);
...
var isFunctionTuple = function (a) {
    return _.isArray(a) &&
        a.length === 2 &&
        _.isObject(a[0]) &&
        _.isFunction(a[1]);
...
var serializeFunctions = function (x) {
    if (_.isFunction(x)) {
        x = x.toString();
...
var serializeMethods = function (o) {
    var v;
